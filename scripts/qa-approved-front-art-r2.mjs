#!/usr/bin/env node
/** Full-HD proof for the exact approved Starhaven menu/loading paintings. */
...
const DEFAULT_OUT = '/tmp/starhaven-approved-front-art-r2';
...
function coversFullHd(width, height) {
  // The art intentionally overscans by up to ~1% so stepped camera drift never exposes an edge.
  return width >= VIEWPORT.width - 2 && height >= VIEWPORT.height - 2
    && width <= VIEWPORT.width * 1.03 && height <= VIEWPORT.height * 1.03;
}

async function canvasSnapshot(page, selector) {
  return page.locator(selector).evaluate((canvas) => canvas.toDataURL('image/png'));
}

async function proveAnimation(page, selector, label) {
  const first = await canvasSnapshot(page, selector);
  await page.waitForTimeout(360);
  const second = await canvasSnapshot(page, selector);
  assertThat(first !== second, `${label}: stepped pixel effects did not animate`);
}
...
async function captureTheme(browser, baseUrl, out, theme) {
  ...
  try {
    ...
    const menu = await page.evaluate((expected) => {
      ...
      const legacy = screen?.querySelector('.menu-list');
      const cover = screen?.querySelector('.front-pixel-build');
    ...
    assertThat(coversFullHd(menu.viewportWidth, menu.viewportHeight), `${theme}: menu cover is ${menu.viewportWidth}×${menu.viewportHeight}`);
    assertThat(menu.effectsWidth === 960 && menu.effectsHeight === 540, `${theme}: menu effects are not 960×540`);
    assertThat(menu.primary?.x > 1400 && menu.primary?.width > 350 && menu.primary?.height > 55, `${theme}: painted New Skirmish target is misaligned`);
    assertThat(menu.start?.x > 1400 && menu.start?.y > 760 && menu.start?.width > 350, `${theme}: painted Start target is misaligned`);
    assertThat(menu.cover?.x < 1580 && menu.cover?.y > 960 && menu.cover?.width > 440, `${theme}: source-label cover is too small`);
    assertThat(menu.legacyOpacity === '0', `${theme}: legacy menu remains visibly layered`);
    assertThat(!menu.containsRts, `${theme}: live menu DOM exposes an RTS subtitle`);
    await proveAnimation(page, '#start-screen .front-approved-effects', `${theme} menu`);
    ...
    const loading = await page.evaluate((expected) => {
      ...
      return {
        ...
        effectsWidth: effects instanceof HTMLCanvasElement ? effects.width : 0,
        effectsHeight: effects instanceof HTMLCanvasElement ? effects.height : 0,
        progress: screen?.querySelector('.front-loading-progress b')?.textContent?.trim() ?? '',
    ...
    assertThat(coversFullHd(loading.viewportWidth, loading.viewportHeight), `${theme}: loading cover is ${loading.viewportWidth}×${loading.viewportHeight}`);
    assertThat(loading.effectsWidth === 960 && loading.effectsHeight === 540, `${theme}: loading effects are not 960×540`);
    assertThat(Number.parseInt(loading.progress, 10) > 12, `${theme}: stepped progress did not advance (${loading.progress})`);
    assertThat(loading.state === 'Loading', `${theme}: loading hold drifted to ${loading.state}`);
    assertThat(!loading.containsRts, `${theme}: loading DOM exposes an RTS subtitle`);
    await proveAnimation(page, '#front-loading-screen .front-approved-effects', `${theme} loading`);
...
async function main() {
  ...
  let server = null;
  let browser = null;
  const manifest = { tool: 'qa-approved-front-art-r2', startedAt: new Date().toISOString(), finishedAt: null, viewport: VIEWPORT, baseUrl: null, themes: [], errors: [], ok: false };
  ...
  try {
    ...
    for (const theme of THEMES) {
      ...
      console.log(`qa-approved-front-art-r2: ${theme} ok=${result.ok}`);
    }
    ...
    assertThat(menuHashes.size === THEMES.length, `approved menu variants are not distinct (${menuHashes.size}/4)`);
    assertThat(loadingHashes.size === THEMES.length, `approved loading variants are not distinct (${loadingHashes.size}/4)`);
  ...
  if (!manifest.ok) {
    ...
    console.log(`PASS qa-approved-front-art-r2: approved paintings, controls, and stepped animation; evidence ${out}`);
  }
}
